...
(defmacro with-circularity-detection ((var &optional stream) &body body)
  ...
  *CIRCULARITY-DETECTION-STREAM-STREAM*, which
  ...
  `(let* ((*circularity-detection-stream-stream* ,(or stream var))
     ...
     (assert (null *circularity-detection-stream*) ()
             "Nesting not implemented.")
...
(defmacro %with-circularity-detection ((stream) &body body)
  ...
  ;;
  ;; would be tempting, but it runs into corrupted output on SBCL. As
  ;; far as I can tell, it's because the pretty-printing stream it
  ;; bind STREAM to buffers stuff.
