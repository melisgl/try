...
* TODO test elisp
* MAYBE support testing for warnings during compilation
CCL for example [handle][pax:clhs]s even user-signalled warnings
during compilation. Textual matching?
* MAYBE limit the number of :HANDLER uses?
It is easy to get into a infinite loop with :HANDLER #'CONTINUE.
* MAYBE make test.sh public
